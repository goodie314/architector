...
describe('DescribedList module', () => {
    ...
    describe('get', () => {
        test('gets an element by id', () => {
            ...
            describedList.push('1', new Describer('div').id('elemId'));
    ...
    describe('push', () => {
        test('adds an element to the list after parent is available', (done) => {
            ...
            describedList
                .push('1', new Describer('div').id('elemId'))
                .then(() => {
                    expect(describedList.length).toEqual(1);
                    done();
                });
            DescribedList.Build(describedList, document.body);
        });

        test('throws error when id is already in the list', async () => {
            const describedList = new DescribedList();
            describedList.push('1', new Describer('div')).then();

            DescribedList.Build(describedList, document.createElement('div'));

            await expect(
                describedList.push('1', new Describer('div')),
    ...
    describe('replaceById', () => {
        test('replaces element in the list with new one', async () => {
            const list = new DescribedList();
            DescribedList.Build(list, document.body);
            await list.push('1', new Describer('div').id('oldId'));

            expect(document.body.firstElementChild.id).toEqual('oldId');
            expect(list.get('1').id).toEqual('oldId');

            await list.replaceById('1', new Describer('div').id('newId'));
        ...
        test('throws error when id does not exist', async () => {
            ...
            DescribedList.Build(list, document.body);
            await expect(
                list.replaceById('1', new Describer('div')),
            ).rejects.toThrow(
    ...
    describe('replaceByIndex', () => {
        test('correctly replaces the nth element', async () => {
            const list = new DescribedList();
            DescribedList.Build(list, document.body);
            await list.push('1', new Describer('div').id('oldId1'));
            await list.push('2', new Describer('div').id('oldId2'));
            ...
            await list.replaceByIndex(1, new Describer('div').id('newId'));
        ...
        test('throws error when index is out of bounds', async () => {
            const list = new DescribedList();
            DescribedList.Build(list, document.body);
            await list.push('1', new Describer('div'));

            await expect(
                list.replaceByIndex(-1, new Describer('div')),
            ).rejects.toThrow(
                ErrorMessages.DescribedList.replaceByIndexError(-1),
            );
            await expect(
                list.replaceByIndex(3, new Describer('div')),
            ).rejects.toThrow(
    ...
    describe('set', () => {
        test('creates a list from an empty list after render', async () => {
            const list = new DescribedList<number>();
            DescribedList.Build(list, document.body);
            const items = Array(2)
                .fill(0)
                .map((n, i) => ({
                    id: i,
                    describer: new Describer('div'),
                }));
        ...
        test('replacing list with more items just concatenates to the end', async () => {
            ...
            let items = [
                {
                    id: 1,
                    describer: new Describer('div').ref(ref1),
                },
                {
                    id: 2,
                    describer: new Describer('div').ref(ref2),
                },
            ];
            await list.set(items);
    ...
    describe('from', () => {
        test.skip('create list from descriptor list', () => {
            ...
            const list = DescribedList.from(items);
